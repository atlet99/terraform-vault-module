...
| <a name="input_quota_rate_limits"></a> [quota\_rate\_limits](#input\_quota\_rate\_limits) | A map of rate limit quotas to configure. | <pre>map(object({<br/>    name           = string<br/>    path           = optional(string)<br/>    rate           = number<br/>    interval       = optional(number)<br/>    block_interval = optional(number)<br/>    role           = optional(string)<br/>    inheritable    = optional(bool)<br/>    group_by       = optional(string)<br/>    namespace      = optional(string)<br/>  }))</pre> | `{}` | no |
